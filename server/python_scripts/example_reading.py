...
def read_example_metagenome_2(here):
    ...
    try:
        table_data, classification_dict = read_metadata(
            here / "data" / "metagenome_three" / "metadata_metagenome.csv", "taxa", network.nodes)
    ...
    try:
        with open(here / "data" / "metagenome_three" / "metagenome_important_nodes.txt", "r") as f:
            important_nodes = [line.strip() for line in f]
            print("Loaded relevant_proteins ", len(important_nodes))
    ...
    try:
        with open(here / "data" / "metagenome_three" / "edges_classification.tsv", "r") as f:
