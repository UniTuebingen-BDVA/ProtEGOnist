...
export const colorScaleAtom = atom((get) => {
    return d3.scaleLinear<string, number>().range(["white", "black"]).domain(get(numEdgesMinMax))
});
export const nodeRadiusAtom = atom((get) => {
    return get(minRadiusAtom) > get(graphAtom).maxradius ? get(graphAtom).maxradius : get(minRadiusAtom)
});
export const centerPointAtom = atom((get) => {
    return ({x: get(graphSizeAtom) / 2 - get(nodeRadiusAtom), y: get(graphSizeAtom) / 2 - get(nodeRadiusAtom)})
});